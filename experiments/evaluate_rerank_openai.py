# openai_reranking_comparison.py
import os
import sys
import time
import pandas as pd
...
from src.data_processing._Tokenization_Embedding import get_openai_tokenizer
...
class OpenAIReRankingTester:
    """
    Focused comparison of OpenAI text-embedding-3-large with and without re-ranking
    """
    
    def __init__(self, papers_list, base_dir="data/raw"):
        """
        Initialize the OpenAI re-ranking tester
        ...
        self.bad_paper_log = "bad_papers_openai_rerank.txt"
        self.collection_name = "openai-large-300tokens-rerank-test"
    ...
    def get_openai_config(self):
        """Get the OpenAI text-embedding-3-large configuration"""
        return {
            'model_type': 'openai',
            'max_tokens': 300,
            'overlap': 50,
    ...
    def get_openai_embedding(self, text: str) -> np.ndarray:
        ...
        try:
            ...
            return np.zeros(3072)  # text-embedding-3-large has 3072 dimensions
    
    def process_papers_openai(self):
        """
        Process papers with OpenAI text-embedding-3-large configuration
        """
        config = self.get_openai_config()
        
        print(f"Processing {len(self.papers_list)} papers with OpenAI {self.embedding_model}")
        print("="*80)
        ...
        for paper in self.papers_list:
            ...
            try:
                ...
                # Chunking - using OpenAI tokenizer
                ...
                print(f"Embedding chunks for paper: {paper} (using {self.embedding_model})")
                all_embeddings = []
        ...
        print(f"Completed OpenAI embedding processing")
        print("="*80 + "\n")
    ...
    def query_without_rerank(self, user_question: str, top_k: int = 10) -> List[Dict]:
        """
        Standard vector similarity search without re-ranking using OpenAI embeddings
        """
        start_time = time.time()
        
        # Create embedding for query using OpenAI
        query_embedding = self.get_openai_embedding(user_question)
    ...
    def run_reranking_comparison(self):
        """
        Run comprehensive comparison of OpenAI embeddings with and without re-ranking
        """
        test_queries = self.create_test_queries()
        comparison_results = []
        
        print(f"Running re-ranking comparison with {len(test_queries)} queries using {self.embedding_model}")
        print("="*80)
    ...
    def visualize_reranking_comparison(self):
        ...
        axes[0,0].set_title(f'Relevance Score: With vs Without Re-ranking\n({self.embedding_model})')
    ...
    def generate_reranking_report(self) -> str:
        ...
        report = f"""
                    # OpenAI Embedding Re-ranking Impact Analysis Report
                    ...
                    4. **OpenAI Model Performance**:
                    - text-embedding-3-large provides high-dimensional (3072D) embeddings
                    - Better semantic understanding compared to domain-specific models for general queries
                    - Cost considerations: ~$0.00013 per 1K tokens
                    ...
                    - OpenAI embeddings excel at: General semantic understanding, cross-domain queries
                    """
    ...
    def run_full_reranking_test(self, process_data=True):
        ...
        if process_data:
            print("="*100)
            print(f"PROCESSING PAPERS WITH OPENAI {self.embedding_model.upper()}")
            print("="*100)
            self.process_papers_openai()
        ...
        if results_df is not None:
            ...
            # Save results
            results_df.to_csv("experiments/results/openai_reranking_comparison.csv", index=False)
            print("\nResults saved to 'openai_reranking_comparison.csv'")
...
if __name__ == "__main__":
    ...
    # Initialize re-ranking tester with OpenAI embeddings
    tester = OpenAIReRankingTester(testing_papers)
    ...
    # tester.process_papers_openai()  # Process data
