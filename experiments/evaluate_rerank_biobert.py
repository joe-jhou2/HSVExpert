...
class BioBERTReRankingTester:
    """
    Focused comparison of BioBERT-300tokens with and without re-ranking
    """
    
    def __init__(self, papers_list, base_dir="data/raw"):
        """
        Initialize the BioBERT re-ranking tester
        ...
        self.collection_name = "biobert-300tokens-rerank-test"
        ...
        # Load BioBERT model
        self.biobert_tokenizer, self.biobert_model = load_embedding_model("dmis-lab/biobert-base-cased-v1.1")
    ...
    def get_biobert_config(self):
        ...
        return {
            ...
            'overlap': 50,
            'max_length': 500,
            'embed_model_name': 'biobert-base-cased-v1.1',
            'collection_name': self.collection_name
        }
    
    def process_papers_biobert(self):
        """
        Process papers with BioBERT-300tokens configuration
        """
        config = self.get_biobert_config()
        
        print(f"Processing {len(self.papers_list)} papers with BioBERT-300tokens")
        print("="*80)
        ...
        for paper in self.papers_list:
            ...
            try:
                ...
                all_embeddings = []
                for chunk in chunks:
                    embedding = embed_text(chunk["text"], self.biobert_tokenizer, self.biobert_model, max_length=config['max_length'])
        ...
        print(f"Completed BioBERT processing")
        print("="*80 + "\n")
    ...
    def query_without_rerank(self, user_question: str, top_k: int = 10) -> List[Dict]:
        ...
        query_embedding = embed_text(user_question, self.biobert_tokenizer, self.biobert_model, max_length=500)
    ...
    def generate_reranking_report(self) -> str:
        ...
        report = f"""
                    # BioBERT Re-ranking Impact Analysis Report
    ...
    def run_full_reranking_test(self, process_data=True):
        ...
        if results_df is not None:
            ...
            # Save results
            results_df.to_csv("experiments/results/biobert_reranking_comparison.csv", index=False)
            print("\nResults saved to 'biobert_reranking_comparison.csv'")
...
if __name__ == "__main__":
    ...
    tester = BioBERTReRankingTester(testing_papers)
